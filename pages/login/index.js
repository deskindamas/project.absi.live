  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://test.tawasyme.com/api/admin/login",
        {
          email,
          password,
        }
        ...
        <div className="flex flex-col justify-start items-start gap-2 w-[80%]">
          ...
          <ul class="grid w-full gap-6 md:grid-cols-2 ">
            <li>
              ...
              <label
                for="customer"
                class="inline-flex items-center justify-center w-full px-3 py-2 text-gray-500 bg-white border border-gray-500 rounded-lg cursor-pointer peer-checked:border-orange-500 peer-checked:text-orange-500 hover:text-gray-600 hover:bg-gray-100 transition-all duration-500"
              >
                <div class="block">
                  <div class="w-full text-lg font-semibold">Customer</div>
                </div>
              </label>
            </li>
            <li>
