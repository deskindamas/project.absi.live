  ...
  const functionopenpopup = async () => {
    ...
    return(
        ...
        <div className="h-screen">
          <div className="m-5 p-5">
            ...
            <div className="w-[50%] flex justify-end ">
              <button  onClick={functionopenpopup} className="bg-skin-primary text-white py-1 px-3 rounded-md">Add Brands</button>
            </div>
            </div>
          </div>
          ...
          <div className="w-full h-[70%] overflow-x-auto ">
              <table className="w-full overflow-x-auto table-auto">
                  ...
                  <tbody className="text-lg font-normal text-gray-700 text-center">
                    {brands.map((brand) => {
                      return <BrandAdmin names={brand} key={brand.id} refetch={() => {refetch();}} />;
                    })}
                  </tbody>
                </table>
        ...
        />
          ...
          <button className="bg-skin-primary rounded-md text-white px-8 py-2">Add Brand</button>
          </div>
           </form>
            </Stack>
        </DialogContent>
