...
function Variations({publicProduct}) {

    const [selectedAttributes, setSelectedAttributes] = useState([]);

    const isButtonDisabled = Object.values(selectedAttributes).some((value) => !value);
    ...
    return (
        <>
            <div>
                {Object.keys(groupedVariations).map((key, index) => {
                    return (
                        <>
               { publicProduct == false ? <div key={index} className="flex flex-col">
               <p>{key}</p>
                     ...
                     onChange={(event) => {
                      setSelectedAttributes({
                        ...
                        checked={selectedAttributes[key] === item.option}
                         />
                        <button
                      className={`border-2 border-orange-500 rounded-lg px-3 py-1 text-gray-700 ${selectedAttributes[key] === item.option ? "opacity-100" : "opacity-60 "} `}
                        onClick={() => {
                     setSelectedAttributes({
                      ...selectedAttributes,
                       [key]: item.option,
     ...
     <p className={` px-3 py-1 text-skin-primary border-b-2 cursor-default `}>{key}</p>
     {groupedVariations[key].map((item) => {
      return (
      <div
